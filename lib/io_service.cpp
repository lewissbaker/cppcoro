...
#if CPPCORO_OS_LINUX
	detail::safe_handle m_wakeupfd;
	detail::safe_handle m_timerfd;
	detail::safe_handle m_epollfd;
#endif
...
void cppcoro::io_service::schedule_impl(schedule_operation* operation) noexcept
{
	...
	const bool ok = m_mq.enqueue_message(
		reinterpret_cast<void*>(operation->m_awaiter.address()), detail::lnx::RESUME_TYPE);
#endif
	if (!ok)
	{
	...
	while (true)
	{
		...
		else if (ok)
		{
			if (completionKey != 0)
			{
				...
				// io_service::schedule().
				stdcoro::coroutine_handle<>::from_address(reinterpret_cast<void*>(completionKey))
					.resume();
				return true;
			}
...
#else
    (void)m_mq.enqueue_message(NULL, detail::lnx::RESUME_TYPE);
#endif
}
...
#elif CPPCORO_OS_LINUX
	: m_wakeupfd(detail::lnx::create_event_fd())
	, m_timerfd(detail::lnx::create_timer_fd())
	, m_epollfd(detail::lnx::create_epoll_fd())
#endif
...
void cppcoro::io_service::timer_thread_state::run() noexcept
{
	...
	const HANDLE waitHandles[waitHandleCount] = { m_wakeUpEvent.handle(),
		...
		if (!timerQueue.is_empty())
		{
			...
			if (!timerQueue.is_empty())
			{
				...
				if (earliestDueTime != lastSetWaitEventTime)
				{
					...
					dueTime.QuadPart =
						-std::chrono::duration_cast<ticks>(timeUntilNextDueTime).count();
